...
$check = "SELECT 1to1_ID FROM BossInvader.U_1to1 WHERE User_ID = '{$createur_ID}';";
$res = $bdd->query($check);
$final = $res->fetch(PDO::FETCH_ASSOC);

$table_ID = $final['1to1_ID'];

$check = "SELECT Participants,Inscrits FROM BossInvader.Tables1to1 WHERE ID = '{$table_ID}';";
$res = $bdd->query($check);
$final = $res->fetch(PDO::FETCH_ASSOC);
...
if ($inscrits != $particpants) {
    $inscrits += 1;
    $query = "UPDATE BossInvader.Tables1to1 SET Inscrits = '{$inscrits}' WHERE Email = '{$email}';";
    $prep = $bdd->prepare($query);
    $exe = $prep->execute();
    
    $stmt = $bdd->prepare("INSERT INTO BossInvader.U_1to1 (User_ID, 1to1_ID) VALUES ('{$user_ID}', '{$table_ID}');");
    $test = $stmt->execute();
